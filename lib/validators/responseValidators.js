var getCurrentDate = require('../currentDate').getCurrentDate;
...
/**
 * Checks if response expires is in the future, allowing it to be cached.
 *
 * @see RFC 2616 Section 14.9.
 * @param {ResponseChecker} checker
 * @returns {{storable: boolean}|undefined}
 */
exports.expiresFuture = function (checker) {
    if (checker.headers['expires'] && new Date(checker.headers['expires']) > getCurrentDate()) {
        return {
            storable: true
        };
    }
